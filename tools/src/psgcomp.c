...
int main (int argc, char *argv[]) {
  ...
  // locate loop marker, if present
  for (i=0;i<size;i++) {
    if (buf[i]==PSG_LOOPMARKER) {
      if (loop_marker_pos!=-1)
        printf ("Warning: multiple loop start markers located. Source file is probably corrupt.\n");
      loop_marker_pos=i;
    }
  }
    ...
    for (skip=0; skip<MAX_LEN; skip++) {
      ...
      if (max_save<(MAX_LEN-3-skip)) {
        ...
        for (haystack=0; haystack<=(consolidate+skip-MAX_LEN); haystack++) {   // less or equal, not only less than
          ...
          // avoid compressing this part if the loop marker falls inside
          if (!((loop_marker_pos>=needle) && (loop_marker_pos<(needle+m_len)))) {
